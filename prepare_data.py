...
# Cluster the data to find patterns in emotions
k = 7
kmeans = KMeans(n_clusters=k, random_state=42, n_init=20)
cluster_ids = kmeans.fit_predict(X_train_scaled)

# Attach cluster IDs back to the DataFrame
train_merged["cluster"] = cluster_ids
...
# Simple cluster purity: for each cluster, majority emotion fraction
cluster_purity = []
for c in range(k):
    sub = train_merged[train_merged["cluster"] == c]
    if len(sub) == 0:
        cluster_purity.append(0.0)
        continue
    majority_frac = sub["Emotion"].value_counts(normalize=True).iloc[0]
    cluster_purity.append(majority_frac)

# Attach cluster IDs to your dataframe for convenience
train_merged["cluster"] = cluster_ids
...
sad_aligned_df = train_aligned[train_aligned["cluster"] == 0]
joy_aligned_df = train_aligned[train_aligned["cluster"] == 1]
...
output_dir = "./data/"
arrays_to_save = {
    "X_sad_audio": X_sad_audio,
    "X_joy_audio": X_joy_audio,
    "X_sad_text": X_sad_text,
    "X_joy_text": X_joy_text,
    "X_sad_text_ref": X_sad_text_ref,
    "X_joy_text_ref": X_joy_text_ref,
}
